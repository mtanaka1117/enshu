...
def get_errors(dataset_name: str, widths: List[int], non_fractional: int) -> List[float]:
    ...
    for width in widths:
        ...
        flattened = inputs.T.reshape(-1).astype(float)
        fixed_point = array_to_fp(flattened, width=width, precision=precision)
        quantized = array_to_float(fixed_point, precision=precision)
        unshifted_error = mean_absolute_error(y_true=flattened, y_pred=quantized)
