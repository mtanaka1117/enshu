...
uint16_t encode_group(uint8_t *output, struct Vector *featureVectors, struct BitMap *collectedIndices, uint16_t numCollected, uint16_t numFeatures, uint16_t seqLength, uint16_t targetBytes, uint16_t precision, FixedPoint *tempBuffer, int8_t *shiftBuffer, uint8_t isBlock);
