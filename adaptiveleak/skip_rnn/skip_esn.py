...
class SkipESNCell(tf.compat.v1.nn.rnn_cell.RNNCell):

    def __init__(self, units: int, input_size: int, sparsity: float, target: float, name: str):
        self._units = units
        self._input_size = input_size
        self._target = target

        # Create a sparsity pattern for the reservoir
        self._rand = np.random.RandomState(seed=53)
        self._pattern = (self._rand.uniform(low=0.0, high=1.0, size=(input_size + units, units)) < sparsity).astype(float)

        # Make the trainable variables for this cell
        with tf.compat.v1.variable_scope(name):
            self.W = tf.compat.v1.get_variable(name='W',
    ...
    def __call__(self, inputs: tf.Tensor, state: SkipRNNStateTuple, scope=None) -> Tuple[SkipRNNOutputTuple, SkipRNNStateTuple]:
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            reservoir = tf.stop_gradient(self.W * self._pattern)  # [K + D, D]
            next_cell_state = tf.matmul(stacked, reservoir)  # [B, D]
...
class SkipESN(NeuralNetwork):
    ...
    def make_graph(self, is_train: bool):
        ...
        # Create the RNN Cell
        self.esn_cell = SkipESNCell(units=self._hypers['rnn_units'],
                               input_size=self.input_shape[-1],
                               target=self.target,
                               sparsity=0.1,
                               name='rnn-cell')

        initial_state = self.esn_cell.get_initial_state(inputs=inputs,
                                                        batch_size=tf.shape(inputs)[0],
                                                        dtype=tf.float32)

        # Apply the RNN Cell [B, T, D]
        esn_output, _ = tf.compat.v1.nn.dynamic_rnn(cell=self.esn_cell,
                                                    inputs=inputs,
                                                    initial_state=initial_state,
                                                    dtype=tf.float32)

        # Extract the output values
        predictions = esn_output.output
        skip_gates = tf.squeeze(esn_output.state_update_gate, axis=-1)
    ...
    def make_loss(self):
        ...
        # Prevent gradients from flowing to the reservoir
        #loss = tf.stop_gradient(loss, self.esn_cell.W)

        self._ops[LOSS_OP] = loss
