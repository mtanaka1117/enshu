...
def plot(sim_results: Dict[str, SimResult], dataset_name: str, output_file: Optional[str]):
    ...
    with plt.style.context('seaborn-ticks'):
        ...
        for name in MODEL_ORDER:
            ...
            r = sim_results[name]
            ax.plot(r.targets, r.size, marker='o', linewidth=4, markersize=8, label=to_label(r.name), color=COLORS[r.name])

        ax.set_xlabel('Fraction of Measurements')
        
        ax.set_ylabel('Message Size (Bytes)')

        ax.set_title('Average Message Sizes on the {0} Dataset'.format(dataset_name.capitalize()))
...
def extract_results(folder: str) -> SimResult:
    ...
    for file_name in sorted(os.listdir(folder)):
        ...
        attack = serialized['attack']['test_accuracy'] if 'attack' in serialized else 0.0
